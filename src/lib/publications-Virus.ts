import { principalInvestigator, teamMembers } from "@/lib/content";
...
function getOrcidRecordIds(): string[] {
  const pratheep = teamMembers.find((member) => member.id === "annamalai")?.orcid;
  return [...new Set([principalInvestigator.links.orcid, pratheep].filter(Boolean).map((entry) => normalizeOrcidId(entry!)))];
}

/**
 * Publications loaded from public ORCID records (PI + Pratheep).
